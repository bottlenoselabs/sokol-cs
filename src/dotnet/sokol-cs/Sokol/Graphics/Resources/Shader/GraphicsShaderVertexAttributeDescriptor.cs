...
namespace Sokol
{
    ...
    public unsafe struct GraphicsShaderVertexAttributeDescriptor
    {
        ...
        public string Name
        {
            get => Native.GetStringFromBytePointer(_name);
            set => _name = Native.GetBytePointerFromString(value);
        }

        /// <summary>
        ///     Gets or sets the name of the HLSL vertex attribute. Required for <see cref="GraphicsBackend.Direct3D11" />. Not used for
        ...
        {
            get => Native.GetStringFromBytePointer(_name);
            set => _semanticName = Native.GetBytePointerFromString(value);
        }
    }
}
