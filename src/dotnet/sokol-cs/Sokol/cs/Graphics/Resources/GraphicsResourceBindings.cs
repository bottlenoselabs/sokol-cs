...
namespace Sokol.Graphics
{
    ...
    public unsafe struct GraphicsResourceBindings
    {
        ...
        [FieldOffset(4)]
        private fixed uint _vertexBuffers[GraphicsConstants.MaximumShaderStageBuffers];

        [FieldOffset(36)]
        private fixed int _vertexBufferOffsets[GraphicsConstants.MaximumShaderStageBuffers];
        ...
        [FieldOffset(76)]
        private fixed uint _vsImages[GraphicsConstants.MaximumShaderStageImages];

        [FieldOffset(124)]
        private fixed uint _fsImages[GraphicsConstants.MaximumShaderStageImages];
        ...
        /// </summary>
        /// <param name="slot">
        ///     The zero-based slot indicating what vertex <see cref="GraphicsBuffer" /> to use. Can't be
        ///     negative. Must be <c>8</c> or less. Default is <c>0</c>.
        /// </param>
        /// <returns>A vertex <see cref="GraphicsBuffer" /> by reference.</returns>
        public readonly ref GraphicsBuffer VertexBuffer(int slot = 0)
        {
            fixed (GraphicsResourceBindings* bindings = &this)
            {
                var ptr = (GraphicsBuffer*)&bindings->_vertexBuffers[0];
                return ref *(ptr + slot);
            }
        }
        ...
        public ref int VertexBufferOffset(int index = 0) => ref _vertexBufferOffsets[index];
        ...
        public readonly ref GraphicsImage VertexStageImage(int index = 0)
        {
            fixed (GraphicsResourceBindings* bindings = &this)
            {
                var ptr = (GraphicsImage*)&bindings->_vsImages[0];
        ...
        public readonly ref GraphicsImage FragmentStageImage(int index = 0)
        {
            fixed (GraphicsResourceBindings* bindings = &this)
            {
                var ptr = (GraphicsImage*)&bindings->_fsImages[0];
                return ref *(ptr + index);
            }
        }
    }
